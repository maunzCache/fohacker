...
const generateLinkedListForLine = function () {
  ...
  while (tmpLinkedList.getNodeValueLength() < maxValueLength) {
    ...
    if (
      (maxValueLength - tmpLinkedList.getNodeValueLength()) >= maxPasswordLength
    ) {
    ...
    if (
      (tmpNodeTypeChance >= 1) && (tmpNodeTypeChance <= nodeTypeChance["dud"])
    ) {
      randomType = "dud"; // TODO: Should access nodeTypes
      nodeValue = "{";
    } else if (
      (tmpNodeTypeChance >= (nodeTypeChance["dud"] + 1)) &&
      (tmpNodeTypeChance <= (nodeTypeChance["dud"] + nodeTypeChance["dummy"]))
    ) {
      randomType = "dummy";
      nodeValue = "a";
    } else {
  ...
  if (tmpLinkedList.getNodeValueLength() > maxValueLength) {
    console.error(
      "List has nodeValueLength",
      tmpLinkedList.getNodeValueLength(),
      "but expected",
      maxValueLength,
    );
  }
