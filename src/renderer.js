...
class Renderer {
  ...
  static createPointersForEachTerminalLinenumber(terminalData, lineNumber) {
    ...
    $(".terminal .linenumber").each(function (_key, value) {
      for (let i = 0; i < terminalData.rowsPerColumn; i++) {
        lineNumber =
          (parseInt(lineNumber, terminalData.rowsPerColumn) + (i * 12))
            .toString(16).toUpperCase(); // TODO: allow overflow!
  ...
  static addFOHackerVersionBeforeInfoInput() {
    $(".info .input").before(
      "Fallout Hacker<br>Version " + Game.version + "<br>",
    );
  }
  ...
  static appendPerkBoxToPerklist(classes, key, perk, description) {
    $(".perklist").append(
      '<div class="' + classes.join(" ") + '" data-id="' + key + '" title="' +
        perk.description[0] + '">' +
        "<b>" + perk.title + "</b> (" + perk.level + "/" + perk.maxlevel +
        ")<br>" +
